"""Custom scraper for Town of Sidney.

Source URL: https://www.sidney.ca/government/
Sidney publishes council agendas and minutes on their government page.
"""
...
class SidneyScraper(BaseScraper):
    """Scraper for Town of Sidney council agendas and minutes."""

    async def discover(self) -> list[DiscoveredItem]:
        """Discover meeting documents from the Sidney government page."""
        ...
        try:
            ...
            # First try to find links to agenda/minutes subpages
            agenda_links = self._find_agenda_links(soup)
            if agenda_links:
                for link_url in agenda_links[:8]:
                    try:
            ...
            logger.error("Sidney scrape failed: %s", e)

        logger.info("Sidney discovery: %d items found", len(items))
        return items

    def _find_agenda_links(self, soup: BeautifulSoup) -> list[str]:
        """Find links to agenda/minutes subpages."""
        links: list[str] = []
        for link in soup.find_all("a", href=True):
            text = link.get_text(strip=True).lower()
            href = link["href"]
            if any(k in text for k in ["agenda", "minutes", "council meeting", "schedule"]):
    ...
    def _parse_page(
        ...
        # Scan list items and content divs
        for el in soup.find_all(["li", "div", "article", "section"]):
            el_text = el.get_text(" ", strip=True)
            meeting_date = self._extract_date(el_text)
