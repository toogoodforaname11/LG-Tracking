...
- **Phases 1, 2, 3, 4 (partial), and 7:** completed in the README rewrite.
- **Phase 4 (artifacts) and Phases 5, 6:** outstanding. Track them as separate work items.
- **Phase 5 license note:** the project has since decided that no public reuse is intended, so no license will be added. The `Add a license file…` bullet below is left in place for historical context only.
